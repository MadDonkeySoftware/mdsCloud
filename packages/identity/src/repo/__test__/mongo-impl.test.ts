...
describe('Mongo Repo Implementation Tests', () => {
  ...
  describe('constructor', () => {
    it('Uses provided connection string', () => {
      // Act
      const client = new MongoRepo('mongodb://localhost');

      // Assert
      expect(client).toBeDefined();
      expect(mockMongoClient).toHaveBeenCalledWith('mongodb://localhost', {});
    });

    it('Uses default connection string', () => {
      // Act
      const client = new MongoRepo();

      // Assert
      expect(client).toBeDefined();
      expect(mockMongoClient).toHaveBeenCalledWith(
        'mongodb://usr:pwd@localhost:27017/identity-db',
        {},
      );
    });
  });
  ...
  describe('handleAppShutdown', () => {
    it('Closes the connection', async () => {
      ...
      // Act & Assert
      await expect(client.handleAppShutdown()).resolves.not.toThrow();
  ...
  describe('getNextCounterValue', () => {
    ...
    it('Throws error when unable to get next sequential value in database', async () => {
      // Arrange
      const findOneAndUpdateStub = jest.fn().mockResolvedValue({ value: null });
      mockMongoClient.mockImplementation(
        () =>
          ({
            connect: jest.fn().mockResolvedValue(undefined),
      ...
      // Act & Assert
      await expect(
        client.getNextCounterValue('foo'),
